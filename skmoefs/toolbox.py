...
# Fork note: toolbox wires ponderaciones through MOEFS, exposes richer outputs in show_model/show_predict, and prints more Pareto diagnostics.
...
class MPAES_RCS(MOEL_FRBC):
    ...
    def _initialize(self, X, y):
        ...
        ponderaciones = self.initializer.get_ponderaciones()
        ...
        # MOEFS problem
        self.problem = RCSProblem(self.Amin, self.M, J, splits, self.objectives, ponderaciones)
        ...
        for i, (train_index, test_index) in enumerate(kf.split(X)):
            ...
            for j in range(len(snapshots)):
                ...
                for k in range(n_sols):
                    ...
                    _=classifier.predict_bm(X_test)
            ...
            for k in range(n_sols):
                ...
                # ant_matrix = classifier.antecedentes()
                # print(k,':', ant_matrix)
                prec, recall, fscore, _ = metrics.precision_recall_fscore_support(y_test_pred, y_test,
                                                                                  average='weighted')
                scores[4, k, i] = prec
    ...
    def show_model(self, position='median', inputs=None, outputs=None, attributes=None , f=None):
        index = self._from_position_to_index(position)
        print('INDICE:', index)
        # index=7
        if index is not None:
            # self.classifiers[index].del_rule()
            # font= open("reglas.txt")
            self.classifiers[index].NEW_show_RB(inputs, outputs, self.classifiers[index].partitions, self.classifiers[index].cons_vect)
            # self.classifiers[index].show_RB(inputs, outputs)
            self.classifiers[index].show_DB(inputs)
            self.classifiers[index].NEW_show_DB(inputs, attributes, self.classifiers[index].partitions)
            return (self.classifiers[index].result_class(),
        self.classifiers[index].pesos_reglas(), self.classifiers[index].antecedentes_matrix(),
        self.classifiers[index].antecedentes_dict(), self.classifiers[index].matriz_particion(),
        self.classifiers[index].granularidad(), self.classifiers[index].particiones(),
        self.classifiers[index].fuzzy_set(), self.classifiers[index].ponderaciones_())
            
    def show_predict(self, position='median', X=None, y=None):
        index = self._from_position_to_index(position)
        if index is not None:
            # self.classifiers[index].del_rule()
            return self.classifiers[index].auc(X,y), self.classifiers[index].predict_bm(X)
    ...
    def _plot_archive(self, archive, x=None, y=None, label='', marker='o'):
        ...
        # ants=
        print('Conteo_reglas:',values[:, trl_index, 0])
        coords = np.array(
