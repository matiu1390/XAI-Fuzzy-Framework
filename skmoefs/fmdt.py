...
# Fork note: decision nodes now store ponderaciones and rule mining returns them alongside rule vectors.
...
class decisionNode:
    ...
    def __init__(self, feature=-1, isLeaf=False, results=None,
                 child=None, weight=None ,fSet=None, ponderaciones=None):
        """Decision Node.
        """
    ...
    def _csv_ruleMine(self,numFeat, prec=[], ruleArray=None, ponderaciones=None):
        if ruleArray == None:
            ruleArray = []
        if ponderaciones == None:
            ponderaciones = []
        if self.isLeaf:
            if sum(self.results) > 0:
                prec[-1] = np.argmax(self.results) + 1
                ruleArray.append(prec)
                ponderaciones.append(self.results)
            ...
            if self.child is None:
                ...
                for k in range(len(self.child)):
                    ...
                    self.child[k]._csv_ruleMine(numFeat, precL[:], ruleArray, ponderaciones)
    
        return ruleArray, ponderaciones
    
    # def extract_ponderaciones(self):
